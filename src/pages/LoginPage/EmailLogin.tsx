...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    try {
      ...
      const token = response.data.token; // JWT 토큰 가져오기
      ...
      setAuth({
        userEmail: userEmail,
        isLoggedIn: true,
        token: token, // JWT 토큰 저장
      });

      Alert.alert("로그인 성공");
      navigation.navigate("Main", {
        nickname: response.data.user.nickname, // Main으로 nickname 전달
        phoneNumber: response.data.user.phoneNumber, // Main으로 phoneNumber 전달
      });
    } catch (error) {
  ...
  const handleFindEmailNavigation = () => {
    navigation.navigate("FindEmail");
  };

  const handleFindPasswordNavigation = () => {
    ...
    <View style={styles.screenContainer}>
      ...
      <View style={styles.container}>
        ...
        <View style={styles.footer}>
          ...
          <TouchableOpacity onPress={handleFindEmailNavigation}>
            <Text style={styles.footerText}>이메일 찾기</Text>
          </TouchableOpacity>
          <Text style={styles.footerText}>|</Text>
          <TouchableOpacity onPress={handleFindPasswordNavigation}>

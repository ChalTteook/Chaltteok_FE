...
function seededRandom(id: string, salt: number = 0) {
  const hash = hashString(id + salt);
  return (hash % 10000) / 10000;
}
function getStudioDiscount(id: string) {
  return Math.floor(seededRandom(id, 1) * 41) + 10; // 10~50%
}
function getStudioRating(id: string) {
  const r = seededRandom(id, 2) ** 8;
  const rating = 4.3 + r * 0.65;
  return rating.toFixed(2);
}
function getStudioReviews(id: string) {
  return Math.floor(seededRandom(id, 3) ** 2 * 100);
}
function getStudioPrice(id: string) {
